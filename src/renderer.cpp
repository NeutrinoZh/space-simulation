...
ParticleRenderer::ParticleRenderer(std::vector<glm::vec3> _vertices,
    ...
    glBufferData(GL_ARRAY_BUFFER, m_models.size() * sizeof(glm::mat4),
                 &m_models[0], GL_STATIC_DRAW);
...
uint ParticleRenderer::vao() const {
    return m_VAO;
}

uint ParticleRenderer::size() const {
    return m_indices.size();
}

void ParticleRenderer::setPosition(uint _i, glm::vec2 _position) {
...
void ParticleRenderer::draw(glm::mat4 _orthoMatrix, glm::mat4 _viewMatrix) {
